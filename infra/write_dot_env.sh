...
DTS_ENDPOINT_VALUE=$(azd env get-value DTS_ENDPOINT)
if [ -n "$DTS_ENDPOINT_VALUE" ]; then
  echo "DTS_ENDPOINT=$DTS_ENDPOINT_VALUE" >> .env
fi

DTS_TASKHUB_VALUE=$(azd env get-value DTS_TASKHUB)
if [ -n "$DTS_TASKHUB_VALUE" ]; then
  echo "DTS_TASKHUB=$DTS_TASKHUB_VALUE" >> .env
fi
